use bitcoin::{Transaction, Txid, TxMerkleNode};
...
impl TxInclusionProof {
    pub fn construct_from_txids(txids: &[Txid], idx: usize) -> Self {
        ...
        let leaf_layer = txids.iter().map(|x|
            TxMerkleNode::from_byte_array(x.to_byte_array())
        ...
        for i in 1..num_layers {
            let layer = layers[i - 1].chunks(2).map(|x| {
    ...
    pub fn verify_hash_inclusion(&self, leaf_hash: &TxMerkleNode, root: &TxMerkleNode) -> Result<()> {
        ...
        if cur != 0 {
            return Err(anyhow::Error::msg("The proof doesn't include the right number of siblings."));
        }
